...
{ // aktualizacja stanu interfejsu informacji zwrotnej; (mask) - zakres zmienianych bit?w
    ...
    case 4: // PoKeys55 wg Marcela - wersja druga z ko?ca 2012
        ...
        /*
                ...
                if(mask & 0x00100000)	if(iBits & 0x10)	MWD->WriteDataBuff[4] |= 1<<3; 		// nadmiarowy silnik?w trakcyjnych
                ...
                if(mask & 0x00400000)	if(iBits & 0x40)	MWD->WriteDataBuff[3] |= 1<<5; 		// po?lizg
                                                        else 				MWD->WriteDataBuff[3] &= ~(1<<5);
                if(mask & 0x00800000)	if(iBits & 0x80)	MWD->WriteDataBuff[4] |= 1<<5; 		// (nadmiarowy) przetwornicy? ++
                                                        else 				MWD->WriteDataBuff[4] &= ~(1<<5);
                if(mask & 0x01000000)	if(iBits & 0x100)	MWD->WriteDataBuff[1] |= 1<<0; 		// nadmiarowy spr??arki
                ...
                if(mask & 0x10000000)	if(iBits & 0x1000)	MWD->WriteDataBuff[1] |= 1<<4; 		// hasler: ci?nienie w hamulcach
                ...
                if(mask & 0x40000000)	if(iBits & 0x4000)	MWD->WriteDataBuff[1] |= 1<<6; 		// brz?czyk SHP/CA
    ...
    if (iMode == 4)
        if (PoKeys55[0])
        {
                        ...
                        if (Global::fCalibrateOutMax[x] > 0)
                        {
                                ...
                                y = y / Global::fCalibrateOutMax[x]; // sprowadzenie do <0,1> je?li podana maksymalna warto??
                                if (Global::iCalibrateOutDebugInfo == x)
